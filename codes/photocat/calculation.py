...
class PhotocatCalculation(CalcJob):
    """Ensemble band-gap prediction (codes/files/photocat_gap.py) on the
    registered ``photocat`` code -- an env with alignn (and optionally
    megnet / modnet) installed. Not an MLIP job: the cmdline carries only
    the gap-filter parameters."""
    ...
    def prepare_for_submission(self, folder):
        ...
        input_file = os.path.join(settings.files_path, 'photocat_gap.py')
