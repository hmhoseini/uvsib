...
class ElectronicCalculation(CalcJob):
    """AiiDA plugin for the no-DFT electronic / light-harvesting screen.

    Stages ``codes/files/electronic.py`` as ``aiida.py`` and runs it against the
    ``input_structures.json`` handed in through the ``file`` namespace (see that
    script's docstring for the I/O contract). No molecular references / MLIP
    calculators are needed -- this job only calls pretrained ML property models.
    """
    ...
    def prepare_for_submission(self, folder):
        ...
        with open(os.path.join(settings.files_path, "electronic.py"), "r", encoding="utf-8") as f:
            content = f.read()
